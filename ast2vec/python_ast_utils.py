...
import knowledge_tracing.ast2vec.tree as tree
import knowledge_tracing.ast2vec.tree_grammar as tree_grammar
import torch
...
def ast_to_tree(ast_node):
    ...
    if label == 'If':
        ...
        for node in ast.iter_child_nodes(ast_node):
            # ignore some nodes
            if isinstance(node, ast.Load) or isinstance(node, ast.Store) or isinstance(node, ast.Del) or isinstance(
                    node, ast.alias):
                continue
            out_node._children.append(ast_to_tree(node))
        return out_node
...
def parse_asts(srcs, filter_uniques=False, ids=None, ignore_syntax_errors=False, verbose=False):
    ...
    for i in progbar(range(len(srcs))):
        try:
            ast_tree = ast.parse(srcs[i], mode='exec')
    ...
    if filter_uniques:
        ...
        return trees_filtered, failed_index, id_map
    else:
        return trees, failed_index, None


# This is a list of builtin functions in the Python language which are
# all available without imports
_builtin_funs = ['abs', 'delattr', 'hash', 'memoryview', 'set', 'all', 'dict', 'help', 'min', 'setattr', 'any', 'dir',
...
_trivial_nodes = ['Global', 'Pass', 'Break', 'Continue'] + [rhs[0] for rhs in _rules['boolop']] + [rhs[0] for rhs in
                                                                                                   _rules[
                                                                                                       'operator']] + [
                     rhs[0] for rhs in _rules['unaryop']] + _cmp_ops

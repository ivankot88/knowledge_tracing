...
import torch
from knowledge_tracing.ast2vec import rtgae2
from knowledge_tracing.ast2vec import python_ast_utils
import astor
...
def worker(model, shared_array, trees, rank, size):
    for i in tqdm(range(rank, len(trees), size), desc=f'Worker: {rank}', total=(len(trees)-rank)//size):
        if trees[i] == None:
            x = torch.zeros(256)
        else:
            x = model.encode(trees[i])
        shared_array[i, :] = x.detach().numpy()


def encode_trees(model, trees, verbose=False, n_jobs=1):
    ...
    if n_jobs > 1:
        shared_array = make_shared_array((len(trees), DIM_), 'float32', fill=0., lock=False)
        process = []
        trees = np.array(trees)

        for proc_id in range(n_jobs):
            model_ = deepcopy(model)
            proc = mp.Process(target=worker, args=[model_, shared_array, trees, proc_id, n_jobs])
            process.append(proc)
            proc.start()
